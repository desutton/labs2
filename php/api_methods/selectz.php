 ...
 * then parse it and then prepare a JSON statement to be uploaded to a EXTJS
 * Store.
 *
 * Version History:
 * 1.0.0 - Initial release
 * 1.5 - re-purposed for Webix lot of logging now.
 * 1.6 - Added basic tree query as a new function. Made old straight JSON as a
 *        function of standard queries. The nested JSON or Tree JSON is a new
 *        function (starting @100) for a simple parent->child hierarchy.
 *        (2020-04-20)
 *****************************************************************************/
...
///              END LOGGING CODE
/// ////////////////////////////////////////////////////////
    $conn = mysqli_connect($hostname, $username, $password, $DB_NAME) or die("Connection failed: " . mysqli_connect_error());
    /* check connection */
...
// Build array of item references:
    foreach ($data as $key => &$item) {
        $itemsByReference[$item[$theTreeChildID]] = &$item;
    }

// Set items as children of the relevant parent item.
    foreach ($data as $key => &$item)
        if ($item[$theTreeParentID] && isset($itemsByReference[$item[$theTreeParentID]]))
            $itemsByReference [$item[$theTreeParentID]]['data'][] = &$item;
