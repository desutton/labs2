...
var tabbar = {
    ...
    elements: [
        ...
        {view: "button", value: "Reset", width: 85, click: "$$('userDetail').clear()"}
    ]
};
...
webix.ajax().get("/labs2/php/api_methods/SELECTz.php?tableName=users&columnNames=users_firstname,users_lastname,users_name,users_displayName,users_pass,users_employeeID&selectColumn=users_name&selectData=" + $userId + "&dataName=data&select=1", function (text, data) {
    var userAuthor = text;
...
//    console.log("JSON DATA being sent to the server " + theSubmitDataRAW2); //just a debug code
    ...
    window.setTimeout(refreshTheUserData, 1000);
    webix.message({text: "Reloading..."});
}

function refreshTheUserData() {
    console.log("hello");
