...
module.exports = function (app, root) {
    ...
    app.put(root + "/films_ordered/:id", async
    ...
    function (req, res, next) {
        try {
            ...
            await
            films.updateAsync({rank: {$gte: old_rank}}, {$inc: {rank: -1}}, {multi: true});
            await
            films.updateAsync({rank: {$gte: new_rank}}, {$inc: {rank: 1}}, {multi: true});
            await
            films.updateAsync({_id: id}, {$set: {rank: new_rank}});
