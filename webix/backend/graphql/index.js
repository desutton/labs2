...
{
    const data = await
    packages.findOneAsync({_id: args.id});
    data.id = data._id;
    return data;
}
,
...
{
    let data = await
    categories.findAsync({parent: args.id});
    data = data.map(a = > {
        a.id = a._id;
    return a;
})
...
{
    let data = await
    packages.findAsync(args.product ? {product_id: args.product} : {});
    data = data.map(a = > {
        a.id = a._id;
    return a;
})
...
>
    {
        await
        packages.updateAsync({_id: args.package.id}, {$set: {name: args.package.name, url: args.package.url}});
...
>
    {
        const doc = await
        products.insertAsync({name: args.product.name});
...
>
    {
        await
        products.updateAsync({_id: args.id}, {$set: {name: args.product.name}});
        return {status: true};
    }
,
...
>
    {
        await
        products.removeAsync({_id: args.id});
        return {status: true};
    }
}
,
